...
if __name__ == '__main__':
    ...
    # model_char = 'mosaic_ctod_state_false'
    # rospy.loginfo(f"Testing model with char {model_char}")
    conf_file_path = os.path.join(
        current_file_path, model_folder, f"config.yaml")
    ...
    if "mosaic" in model_folder and "_ctod" not in model_folder and '_kp' not in model_folder:
        ...
        ai_controller = CTODController(
            ...
            trj_number=trj_number,
            camera_name='camera_front')
    elif "mosaic_kp" in model_folder or "MOSAIC-KP" in model_folder or "mosaic_cod" in model_folder:
    ...
    while True:
        ...
        while t < max_T and not done and not STOP:
            ...
            if trj is not None:
                ...
                if True: #"rt1_real" in model_folder:imwrite
            ...
            if MOSAIC_CTOD:
                # pass
                if desired_position[2] < 0.10 and not picked and desired_position[0] > 0.0:
            ...
            if gripper_finger_pos == 255 and not picked and not picked_old:
                ...
                if picked and not picked_old:
                    rospy.loginfo("Transition open -> close: Object picked")
                    picked_old = picked
                    if MOSAIC_CTOD:
                        desired_position[2] = desired_position[2] + 0.05 
